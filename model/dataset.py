...
class ProteinTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            img2tensor(img_array).type(torch.FloatTensor).to(self.device),
            torch.from_numpy(target_array).type(torch.FloatTensor).to(self.device),
        )
...
class ProteinTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_fn = self.fn_list[idx]
        img_array = open_rgby(self.path, img_fn).astype(float)
        return img2tensor(img_array).type(torch.FloatTensor).to(self.device)
